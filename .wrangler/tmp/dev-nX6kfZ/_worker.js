  ...
  <header class="hero">
    <div class="top">
      ...
      <div class="top-buttons">
        <button class="head-btn" id="wildcardOpen" type="button" aria-label="Buka list wildcard">
          <svg viewBox="0 0 24 24" fill="none"><path d="M4.5 8.5h15m-15 7h15M8 4.5 5.5 19.5M16 4.5l2.5 15" stroke="currentColor" stroke-width="1.7"/></svg>
          <span>LIST WC</span>
        </button>
    ...
    <div class="info">
      <div class="info-box">
        ...
        <div class="tags"><span class="tag">VLESS</span><span class="tag">TROJAN</span><span class="tag">SHADOWSOCKS</span><span class="tag">WS</span><span class="tag">WC</span></div>
      </div>
    ...
    <div class="path">
      ...
      <div><b>CUSTOM PATH</b><p>Otomatis mengikuti proxy pilihan: <code>/Free-VPN-CF-Geo-Project/IP=PORT</code></p></div>
    </div>
  </header>
...
(() => {
  ...
  function cleanAddress(value) {
    return String(value || "").trim().replace(/^https?:\\/\\//i, "").replace(/^\\/\\//, "").split("/")[0].split(":")[0].trim();
  }
  function wcSubdomain(value) {
    const domain = cleanAddress(value).replace(/^\\*\\./, "").replace(/^\\.+/, "");
  ...
  function statusMarkup(server, index) {
    const s = server.status || "ready";
    const label = {ready:"CHECK",checking:"CHECKING",active:"ACTIVE",inactive:"INACTIVE"}[s];
    return \`<button class="check \${s}" data-check="\${index}"><i></i>\${label}</button>\`;
  }
  function metricMarkup(server) {
    if (!server.metricsFromApi || server.status === "checking") return "";
    return \`<div class="metric"><span>Delay: \${esc(server.delay)}</span><span class="pipe">|</span><span class="speed">Speed: \${esc(server.speed)}</span></div>\`;
  }
  function modeControls(index) {
    const wcText = selectedWildcard ? \`HOST/SNI: \${esc(wcSubdomain(selectedWildcard))}\` : "DOMAIN WC: pilih melalui List Wildcard";
    const detail = selectedMode === "ws"
      ? \`<div class="mode-detail ws-field"><label>BUG WS</label><input class="ws-input" data-ws-bug type="text" value="\${esc(selectedBug)}" placeholder="bug.domain.com (opsional)"><p class="mode-hint">Kosong = menggunakan \${HOST}</p></div>\`
      : \`<div class="mode-detail"><div class="wc-selected">\${wcText}</div><p class="mode-hint">Tekan WC lagi untuk memilih domain lain.</p></div>\`;
    return \`<div class="chooser"><div class="chooser-inner">
      <div class="choose-label">MODE KONEKSI</div>
      <div class="mode-row"><button class="mode \${selectedMode === "ws" ? "active" : ""}" data-mode="ws">WS</button><button class="mode \${selectedMode === "wc" ? "active" : ""}" data-mode="wc">WC</button></div>
      \${detail}
      <div class="choose-label">PILIH PROTOKOL</div>
      <div class="protocol-row"><button class="copy" data-copy="vless" data-index="\${index}">VLESS</button><button class="copy" data-copy="shadowsocks" data-index="\${index}">SS</button><button class="copy" data-copy="trojan" data-index="\${index}">TROJAN</button></div>
    </div></div>\`;
  }
  function render() {
  ...
  function updateCheck(index) {
    ...
    if (a) a.innerHTML = statusMarkup(servers[index], index);
    if (b) b.innerHTML = metricMarkup(servers[index]);
  }
  function refreshChoosers() {
    document.querySelectorAll(".server").forEach((card,index) => {
      const chooser = card.querySelector(".chooser"); if (chooser) chooser.outerHTML = modeControls(index);
    });
  }
  function selectMode(mode) {
    if (mode === "wc") {
      selectedMode = "wc"; selectedWildcard = ""; refreshChoosers(); openWildcard(); toast("Pilih domain untuk mode WC"); return;
    }
    selectedMode = "ws"; refreshChoosers(); toast("Mode WS aktif");
  }
  function makeConfig(server, protocol) {
    if (selectedMode === "wc" && !selectedWildcard) return null;
    const bugHost = cleanAddress(selectedBug);
    const connectHost = selectedMode === "wc" ? cleanAddress(selectedWildcard).replace(/^\\*\\./, "") : (bugHost || HOST);
    const tlsHost = selectedMode === "wc" ? wcSubdomain(selectedWildcard) : HOST;
    const path = "/Free-VPN-CF-Geo-Project/" + server.ip + "=" + server.port;
  ...
  async function copyValue(value, text) {
    ...
    catch (_) { const a=document.createElement("textarea");a.value=value;a.style.position="fixed";a.style.opacity="0";document.body.appendChild(a);a.select();document.execCommand("copy");a.remove(); }
    toast(text);
  }
  async function checkServer(index) {
    const server = servers[index]; if (!server || server.status === "checking") return;
    server.status="checking"; server.delay=""; server.speed=""; server.metricsFromApi=false; updateCheck(index);
    try {
      const res = await fetch("/api/check?ip=" + encodeURIComponent(server.ip + ":" + server.port), {cache:"no-store"});
      const data = await res.json();
      server.status = String(data.status || "").toUpperCase() === "ACTIVE" ? "active" : "inactive";
      server.delay = String(data.delay || "").trim(); server.speed = String(data.speed_est || "").trim();
      server.metricsFromApi = Boolean(server.delay && server.speed);
    } catch (_) { server.status="inactive"; server.metricsFromApi=false; }
    updateCheck(index);
  }
  ...
  function renderWildcards() {
    if (!wildcards.length) { $("wildcardList").innerHTML=\`<div class="message">No wildcard</div>\`; return; }
    $("wildcardList").innerHTML = wildcards.map((wc,index) => \`<div class="wildcard-item"><code>\${esc(wc)}</code><button class="use" data-use-wc="\${index}">GUNAKAN</button><button class="wc-copy" data-copy-wc="\${index}">COPY</button></div>\`).join("");
  }
  async function loadWildcards() {
    try {
      const res=await fetch("/api/wildcards?t="+Date.now(),{cache:"no-store"}); const data=await res.json();
      if(!res.ok) throw new Error(); wildcards=data.wildcards||[]; wildcardLoaded=true; renderWildcards();
    } catch (_) { $("wildcardList").innerHTML=\`<div class="message">Failed to load wildcard</div>\`; }
  }
  function openWildcard(){ $("wildcardModal").classList.add("show"); if(!wildcardLoaded) loadWildcards(); }
  function closeWildcard(){ $("wildcardModal").classList.remove("show"); }
  function openDonate(){ $("donateModal").classList.add("show"); }
  function closeDonate(){ $("donateModal").classList.remove("show"); }

  document.addEventListener("input", event => {
    const input=event.target.closest("[data-ws-bug]"); if(!input) return;
    selectedBug=cleanAddress(input.value); try{localStorage.setItem(BUG_KEY,selectedBug)}catch(_){}
  });
  document.addEventListener("click", async event => {
    ...
    if(useWc){ selectedWildcard=wildcards[Number(useWc.dataset.useWc)]||""; selectedMode="wc"; refreshChoosers(); closeWildcard(); toast("Domain WC dipilih"); return; }
    if(copyWc){ await copyValue(wildcards[Number(copyWc.dataset.copyWc)], "Wildcard copied"); return; }
    if(copy){ const config=makeConfig(servers[Number(copy.dataset.index)],copy.dataset.copy); if(!config){openWildcard();toast("Pilih domain WC");return;} await copyValue(config, copy.dataset.copy.toUpperCase()+" copied"); }
  });
  $("themeToggle").onclick=toggleTheme; $("wildcardOpen").onclick=openWildcard; $("wildcardClose").onclick=closeWildcard;
  $("wildcardModal").onclick=e=>{if(e.target===$("wildcardModal"))closeWildcard()};
  $("donateModal").onclick=e=>{if(e.target===$("donateModal"))closeDonate()};
...
// .wrangler/tmp/bundle-0N3VqU/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0N3VqU/middleware-loader.entry.ts
