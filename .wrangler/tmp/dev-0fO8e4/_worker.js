...
<div id="sub-tab-zone-setup" class="sub-tab-content-zone active fade-in space-y-4 sm:space-y-6">
    ...
    <button onclick="showPagesCustomDomainModal()" class="bg-orange-600 text-white px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-orange-500 transition-all flex items-center gap-1.5">
        <i class="fa-solid fa-globe"></i> Custom Domain
    </button>
...
<\/script>
    ...
    <!-- Pages Custom Domain Modal -->
    <div id="pagesCustomDomainModal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 hidden">
        <div class="bg-[#161b22] border border-slate-800 rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
            <div class="flex justify-between items-center p-6 border-b border-slate-800">
                <h3 class="text-xl font-bold text-white">Add Custom Domain</h3>
                <button onclick="hidePagesCustomDomainModal()" class="text-slate-500 hover:text-white transition-all"><i class="fa-solid fa-times text-xl"></i></button>
            </div>
            <div class="p-6 space-y-4">
                <div>
                ...
                <div>
                    ...
                    <input id="pagesSubdomainPrefix" oninput="updatePagesResultingDomain()" type="text" placeholder="e.g. testing or testing.eu" class="w-full mt-1 bg-[#0d1117] border border-slate-700 rounded-xl px-4 py-2 text-sm focus:border-blue-500 outline-none font-bold text-white">
                    <p class="text-[9px] text-slate-600 mt-1 italic">Leave blank to use the root domain.</p>
                </div>
                <div>
            ...
            <div class="p-6 border-t border-slate-800 bg-[#0d1117] flex justify-end gap-3">
                <button onclick="hidePagesCustomDomainModal()" class="bg-slate-800 hover:bg-slate-700 px-6 py-2 rounded-xl text-xs font-bold text-white transition-all">Cancel</button>
                <button onclick="addPagesCustomDomain()" class="bg-blue-600 hover:bg-blue-500 px-6 py-2 rounded-xl text-xs font-bold text-white transition-all">ADD DOMAIN</button>
            </div>
        </div>
    </div>
    ...
    <div>
        ...
        async function loadPagesLinkedDomains() {
            ...
            try {
                ...
                const data = await res.json();
                if (data.success) {
                    currentPagesLinkedDomains = data.result;
        ...
        function renderPagesLinkedDomains(domains) {
            ...
            if (rootDomain) {
                filteredDomains = domains.filter(function(d) {
                    return d.name === rootDomain || d.name.endsWith('.' + rootDomain);
                });
            }

            if (filteredDomains.length === 0) {
                list.innerHTML = '<div class="text-center py-4 text-slate-600 text-xs italic">No domains linked matching the selected root domain.</div>';
                return;
            }

            list.innerHTML = filteredDomains.map(function(d) {
                let displayName = d.name;
                if (rootDomain && d.name.endsWith('.' + rootDomain)) {
                    displayName = d.name.slice(0, -(rootDomain.length + 1));
                } else if (rootDomain && d.name === rootDomain) {
                    displayName = "@";
                }
        ...
        async function loadPagesRegisteredDomains() {
            ...
            try {
                ...
                const data = await res.json();
                if (data.success && data.result.length > 0) {
                    select.innerHTML = '<option value="">Select a domain</option>' + data.result.map(function(z) {
        ...
        function hidePagesCustomDomainModal() {
            document.getElementById('pagesCustomDomainModal').classList.add('hidden');
        }

        function updatePagesResultingDomain() {
            const domain = document.getElementById('pagesDomainSelect').value;
            ...
            if (!domain) {
                document.getElementById('pagesResultingDomain').value = '';
                renderPagesLinkedDomains(currentPagesLinkedDomains);
                return;
            }

            if (prefix) {
                document.getElementById('pagesResultingDomain').value = prefix + '.' + domain;
            } else {
                document.getElementById('pagesResultingDomain').value = domain;
            }
        ...
        async function addPagesCustomDomain() {
            const customDomain = document.getElementById('pagesResultingDomain').value;
            if (!customDomain) return alert("Please specify a valid custom domain.");
            if (!currentActivePagesProject) return alert("Select a project first!");
            ...
            try {
                const res = await fetch(API_BASE_URL + '/api/pages/addDomain', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        email: acc.email,
                ...
                if (data.success) {
                    ...
                    setTimeout(() => loadPagesLinkedDomains(), 2000);
                } else {
...
// .wrangler/tmp/bundle-ZTmm3J/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-ZTmm3J/middleware-loader.entry.ts
