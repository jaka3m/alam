...
async function getScriptConfig(env, request) {
  ...
  const tempConfig = {
    API_KEY: env.API_KEY || "cfk_OsUtHbEnFPI3kjXvC3fKQSZ1eoHE3AbzCYpPfr1I5e4f482",
    API_EMAIL: env.API_EMAIL || "pikaajamal@gmail.com"
  };
  ...
  if (!cachedProxyList.length || forceReload) {
    ...
    try {
      ...
      if (proxyBank.status === 200) {
        ...
        let pathCounters = {};
        processedConfigs = cachedProxyList.map((config) => {
        ...
        console.log(`Fetched and processed ${cachedProxyList.length} proxies from R2.`);
      } else {
...
var worker_default = {
  async fetch(request, env, ctx) {
    try {
      ...
      if (url.pathname === "/api/v1/proxy-stats") {
        const proxyList = await getProxyList();
        const stats = {
          total: proxyList.length,
          active: 0,
          dead: 0,
          unknown: proxyList.length
        };
...
async function handleWebRequest(request, env, config) {
  ...
  visibleConfigs.forEach((config2, index) => {
    ...
    cardsHTML += `
        <article class="server proxy-row" data-ip-port="${ipPort}" loading="lazy">
            <div class="identity">
  ...
  <select id="wildcard" name="wildcard" onchange="onWildcardChange(event)"
    ...
    ${allWildcards.map((w) => `<option value="${w}" ${selectedWildcard === w ? "selected" : ""}>${w}</option>`).join("")}
      ...
      <section class="servers">
        <div class="server-head flex flex-wrap items-center gap-2 mb-4">
            <h2 class="text-xl font-bold">Server</h2>
            <span class="count" id="count">${totalFilteredConfigs}</span>
            <span class="count bg-green-500/20 text-green-400 border-green-500/30" title="Total Active">
                <i class="fas fa-check-circle mr-1"></i><span id="total-active-count">0</span>
            </span>
            <span class="count bg-red-500/20 text-red-400 border-red-500/30" title="Total Dead">
                <i class="fas fa-times-circle mr-1"></i><span id="total-dead-count">0</span>
            </span>
            <button id="refresh-all-status" class="count bg-blue-500/20 text-blue-400 border-blue-500/30 hover:bg-blue-500/40 transition-all active:scale-95 sm:ml-auto">
        ...
        <\/script>
                ...
                document.addEventListener('DOMContentLoaded', () => {
                    ...
                    const totalDeadEl = document.getElementById('total-dead-count');
                    const refreshBtn = document.getElementById('refresh-all-status');

                    let globalActive = 0;
                    let globalDead = 0;

                    const updateGlobalStatsDisplay = () => {
                        if (totalActiveEl) totalActiveEl.textContent = globalActive;
                        if (totalDeadEl) totalDeadEl.textContent = globalDead;
                    };

                    const updateGlobalStats = async () => {
                        try {
                            const res = await fetch('/api/v1/all-proxy-stats');
                            if (res.ok) {
                                const stats = await res.json();
                                if (document.getElementById('count')) document.getElementById('count').textContent = stats.total;
                            }
                        } catch (e) {
                    ...
                    const checkProxy = async (row) => {
                        ...
                        if (checkWrap) {
                            checkWrap.innerHTML = '<button class="check checking"><i></i>CHECKING</button>';
                        }

                        try {
                            const response = await fetch("/geo-ip?ip=" + ipPort);
                            ...
                            let statusHTML = '';
                            if (status === 'ACTIVE') {
                                if (row.getAttribute('data-status-counted') !== 'ACTIVE') {
                                    if (row.getAttribute('data-status-counted') === 'DEAD') globalDead--;
                                    globalActive++;
                                    row.setAttribute('data-status-counted', 'ACTIVE');
                                    updateGlobalStatsDisplay();
                                }
                                statusHTML = '<button class="check active" onclick="event.stopPropagation(); recheckSingle(this)"><i></i>ACTIVE</button>';
                            } else if (status === 'DEAD') {
                                if (row.getAttribute('data-status-counted') !== 'DEAD') {
                                    if (row.getAttribute('data-status-counted') === 'ACTIVE') globalActive--;
                                    globalDead++;
                                    row.setAttribute('data-status-counted', 'DEAD');
                                    updateGlobalStatsDisplay();
                                }
                                statusHTML = '<button class="check inactive" onclick="event.stopPropagation(); recheckSingle(this)"><i></i>INACTIVE</button>';
                            } else {
                                statusHTML = '<button class="check inactive" style="color: orange; border-color: rgba(255,165,0,.22); background: rgba(255,165,0,.10);" onclick="event.stopPropagation(); recheckSingle(this)"><i></i>UNKNOWN</button>';
                            }
                            ...
                            if (checkWrap) {
                                checkWrap.innerHTML = '<button class="check inactive" style="color: cyan; border-color: rgba(0,255,255,.22); background: rgba(0,255,255,.10);" onclick="event.stopPropagation(); recheckSingle(this)"><i></i>ERROR</button>';
                            }
                            return 'ERROR';
                        }
                    };

                    window.recheckSingle = async (btn) => {
                        const row = btn.closest('.proxy-row');
                        await checkProxy(row);
                    };

                    const checkBatch = async (batch) => {
                        return Promise.all(batch.map(row => checkProxy(row)));
                    };
                    ...
                    const checkAllProxies = async () => {
                        ...
                        for (let i = 0; i < concurrency; i++) {
                            activeChecks.push(processQueue());
                        }

                        await Promise.all(activeChecks);
                    };
                    ...
                    // Background task to count global active/dead (lazy)
                    const calculateGlobalStats = async () => {
                        // In a real scenario, you might want to fetch this from an API or storage
                        // For now, let's simulate by checking some from the list or just use a placeholder
                        // Since we have 10k+ proxies, we can't check all in browser background easily without API support
                    };

                    // Lazy Loading implementation for checking
                            ...
                            entries.forEach(entry => {
                                if (entry.isIntersecting) {
                                    ...
                                    if (row.getAttribute('data-checked') !== 'true') {
                                        ...
                                        // checkProxy(row); // We already have checkAllProxies running
                                    }
                                }
                            });
                        }, { rootMargin: '100px' });
...
// .wrangler/tmp/bundle-0QQb6j/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-0QQb6j/middleware-loader.entry.ts
