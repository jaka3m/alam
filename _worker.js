...
<div x-data="{
    ...
    async loadPage(url, menu) {
        ...
        try {
            ...
            const newContent = doc.querySelector('main.app') || doc.querySelector('.container') || doc.body;
...
}" @keydown.escape.window="sidebarOpen = false" class="relative">
    <script>
    ...
    >
        ...
        <nav class="space-y-1">
            ...
            <a
                ...
                @click.prevent="loadPage('/web', 'create')"
            >
            ...
            <a
                ...
                @click.prevent="loadPage('/vpn', 'converter')"
            >
                <div class="active-indicator"></div>
            ...
            <a
                ...
                @click.prevent="loadPage('/kuota', 'quota')"
            >
                <div class="active-indicator"></div>
            ...
            <a
                ...
                @click.prevent="loadPage('/checker', 'checker')"
            >
                <div class="active-indicator"></div>
        ...
        <a
            ...
            @click.prevent="loadPage('/stats', 'traffic')"
        >
            <div class="active-indicator"></div>
...
async function getProxyList(forceReload = false) {
  ...
  const cacheKey = new Request('https://proxy-list.cache/all');
  ...
  try {
    ...
    if (proxyBank.status === 200) {
      ...
      const response = new Response(JSON.stringify(proxies), {
...
`;
  if (isSpa) {
    ...
    const scriptsMatch = html.match(/(<script\b[^>]*>([\s\S]*?)<\/script>)/g);
...
<script>
    ...
    if (isSpa) {
      ...
      const content = mainMatch ? mainMatch[1] : html;
      const scripts = scriptsMatch ? scriptsMatch.join('\n') : '';
      return new Response(content + scripts, { headers: { "Content-Type": "text/html; charset=utf-8" } });
    }
...
async function handleWebRequest(request, env, config, isSpa = false) {
    const cfApi = new CloudflareApi(config);
    let dynamicDomains = [];
    try {
    ...
    ${allWildcards.map(w => `<option value="${w}" ${selectedWildcard === w ? 'selected' : ''}>${w}</option>`).join('')}
      ...
      <section class="servers">
        <div class="server-head">
            <h2>Server</h2>
            <div class="flex items-center gap-2">
                <div class="count flex items-center gap-2" style="border-color: var(--green); color: var(--green);">
                    <i class="fas fa-check-circle"></i>
                    <span id="active-count">0</span>
                </div>
                <div class="count flex items-center gap-2" style="border-color: var(--red); color: var(--red);">
                    <i class="fas fa-times-circle"></i>
                    <span id="inactive-count">0</span>
                </div>
                <span class="count" id="count">${totalFilteredConfigs}</span>
                <button onclick="checkAllProxies()" class="count hover:bg-mint hover:text-bg transition-colors" title="Cek Ulang Semua">
                    <i class="fas fa-sync-alt"></i> CEK SEMUA
                </button>
            </div>
        </div>
        <div class="list" id="list">
            ...
            document.addEventListener('DOMContentLoaded', () => {
                ...
                async function checkSingleProxy(row) {
                    ...
                    if (checkWrap) {
                        checkWrap.innerHTML = '<button class="check checking"><i></i>CEK...</button>';
                    }
                    ...
                    try {
                        ...
                        const speed = data.speed_est || '-';

                        let statusHTML = '';
                        row.classList.remove('is-active', 'is-inactive');

                        if (status === 'ACTIVE') {
                            statusHTML = '<button class="check active" onclick="event.stopPropagation(); checkSingleProxy(this.closest(\u0027.server\u0027))"><i></i>AKTIF</button>';
                            row.classList.add('is-active');
                        } else {
                            statusHTML = '<button class="check inactive" onclick="event.stopPropagation(); checkSingleProxy(this.closest(\u0027.server\u0027))"><i></i>MATI</button>';
                            row.classList.add('is-inactive');
                        }

                        if (checkWrap) checkWrap.innerHTML = statusHTML;
                        if (metricContainer) {
                            metricContainer.innerHTML = '<span>Delay: ' + delay + 'ms</span><span class="pipe">|</span><span class="speed">Speed: ' + speed + '</span>';
                        }
                    } catch (e) {
                        if (checkWrap) checkWrap.innerHTML = '<button class="check inactive"><i></i>ERROR</button>';
                    }
                    updateCounters();
                }

                function updateCounters() {
                    const active = document.querySelectorAll('.server.is-active').length;
                    const inactive = document.querySelectorAll('.server.is-inactive').length;
...
document.getElementById('search-button').addEventListener('click', executeSearch);
  ...
  if (isSpa) {
    ...
    const content = mainMatch ? mainMatch[1] : html;
    const scripts = scriptsMatch ? scriptsMatch.join('\n') : '';
    return new Response(content + scripts, { headers: { 'Content-Type': 'text/html' } });
  }
